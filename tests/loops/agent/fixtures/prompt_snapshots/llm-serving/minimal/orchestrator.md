...
Require a synthetic successful-row preflight through comparison, enrichment,
and serialization as well as the failed-gate test. Name the execution boundary:
...
remote response is durably written. The implementer must inspect remote
callables for local-workspace artifact reads before spending on measurement.
