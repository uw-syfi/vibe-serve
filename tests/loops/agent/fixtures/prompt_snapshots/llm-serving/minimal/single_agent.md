...
Make the controller fail closed in code. Before remote execution, inject or
synthesize a failed capability/correctness/smoke result and assert that the
downstream representative or canonical callable is not invoked while the
failure artifact is retained. Recording `issues` and continuing is not a gate.
Preflight the success path with a fake representative row through comparison,
enrichment, and final serialization. Remote code may read only files explicitly
bundled or mounted there; pass local baselines as primitive inputs or compare
only after the raw remote response is durably written. Inspect remote callables
