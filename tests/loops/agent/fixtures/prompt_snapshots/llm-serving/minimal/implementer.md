...
Make polling cadence proportional to the remote phase. During model load,
compilation, graph capture, or another minutes-long phase, do not poll a healthy
unchanged PID every few seconds. Start with a 20-30 second interval, back off up
to 60 seconds while state is unchanged, and shorten it only when a checkpoint
shows a short phase or imminent completion. Do not emit repeated unchanged
process trees or log tails; report transitions, new checkpoints, deadline
crossings, and terminal errors. Polling more frequently does not accelerate the
remote work and consumes the context needed for optimization reasoning.
...
that produced it. Before the run, retain an identity that is sufficient to
restore and audit that candidate: use a framework/VCS checkpoint when it covers
the exact measured tree, or otherwise record content hashes for all
behavior-affecting candidate source, build inputs, runtime configuration, and
the built image/artifact identity when the platform exposes one. A hash of one
convenient source file is not
enough for a multi-file or multi-language candidate. If you change candidate
behavior or its execution/build configuration after measurement, the completed
row remains valid historical evidence for its measured checkpoint, but it is not
evidence for the new workspace state. Either restore/preserve the exact measured
checkpoint as the nominated candidate or obtain evidence for the new state when
the hypothesis genuinely requires it. Report-only analysis, documentation, and
derived-view changes do not invalidate an otherwise identified measurement.
Return control once a terminal hypothesis result is ready instead of making an
unmeasured follow-on optimization inside the same persistent turn.
