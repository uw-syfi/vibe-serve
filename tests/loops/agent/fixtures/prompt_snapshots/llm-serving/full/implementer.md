...
logic, serialization, or an execution boundary, make that changed path fail
closed in code: every failed capability, correctness, or smoke gate must return
before the downstream expensive callable is invoked. Before remote execution,
