...
Make staged controllers fail closed in code, not only in prose: every failed
capability, correctness, or smoke gate must return before the downstream
expensive callable is invoked. Before remote execution, inject or synthesize one
