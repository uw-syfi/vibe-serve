...
Detailed constrained-decoding changes:

- The benchmark still sent real `guided_json` requests through vLLM's OpenAI-compatible completion path. The specialized wrapper did not replace generation with canned JSON, did not bypass the tokenizer, and validated every completed response against the schema.
- The Modal wrapper served from a local patched vLLM source tree while preserving the same external API. The image installed `vllm==0.10.0` for compiled artifacts, mounted the local `vllm/` source, preferred that source on `sys.path`, and forwarded `guided_json`, `guided_regex`, `guided_choice`, `guided_grammar`, `guided_json_object`, `guided_decoding_backend`, and `guided_whitespace_pattern` into `GuidedDecodingParams.from_optional`.
- The wrapper also carried serving-parameter changes: tensor parallel size 1, `max_model_len=4096`, `max_num_seqs=64`, `max_num_batched_tokens=8192`, chunked prefill, prefix caching, CUDA graph sizes through 64, one Modal H100 container, `@modal.concurrent(max_inputs=16)`, FlashAttention, and optional FlashInfer sampling. Those settings are part of the measured specialized wrapper, but the ablation below shows they were not the main reason for the constrained speedup.
- The vLLM code patch changed `GPUModelRunner.apply_grammar_bitmask`. Stock vLLM receives grammar bitmasks compacted to only the structured-output requests, maps request ids back to logits rows, builds `indices` for the rows that need masking, copies a row-aligned bitmask to GPU, and calls xgrammar with `indices=...`. That general path is necessary when one batch mixes unconstrained requests and guided JSON requests.
