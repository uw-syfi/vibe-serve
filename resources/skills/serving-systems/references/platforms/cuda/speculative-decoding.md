...
- [`cuda-graph.md`](cuda-graph.md) — capture mechanics, fixed-shape and stable-address requirements
- [`attention-backend-comparison.md`](attention-backend-comparison.md) — which attention backend to use for which phase under graph capture
- [`flashinfer.md`](flashinfer.md) — FlashInfer batched wrappers (graph-safe) and the single-request wrappers (not graph-safe)
- [`algorithms/structured-output.md`](../../algorithms/structured-output.md) — xgrammar interaction, jump-forward, grammar masking
