...
- [`platforms/cuda/async-scheduling`](async-scheduling.md) — hide the CPU scheduler work (pick next batch, build metadata) behind the GPU forward pass; the next step once continuous batching is working
- [`algorithms/paged-attention`](../../algorithms/paged-attention.md) — the KV-storage substrate that makes variable-length continuous batching efficient
- [`algorithms/chunked-prefill`](../../algorithms/chunked-prefill.md) — interleave long-prompt prefill chunks with active decodes
- [`cuda-graph`](cuda-graph.md) — capture the decode step to eliminate kernel-launch overhead
- [`algorithms/batched-sampling`](../../algorithms/batched-sampling.md) — remove per-request CPU-GPU sync on the sampling side
...
**Full coverage, including the paged-KV contract and a minimal KV manager**: [`flashattention.md`](flashattention.md) and its [`#paged-kv-manager`](flashattention.md#paged-kv-manager).
...
**Full coverage of wrappers, layouts, and pitfalls**: [`flashinfer.md`](flashinfer.md).
...
Both require the engine to **drive the per-layer loop directly** — model classes become weight containers, and the engine orchestrates `layernorm → QKV → RoPE → append KV → attention → output` per layer. See [`flashinfer.md`](flashinfer.md) "Engine-drives-layers architecture" for the full pattern.

## Related skills

- [`algorithms/paged-attention`](../../algorithms/paged-attention.md) — the block-pool / page-table design that underlies both options
- [`algorithms/radix-prefix-caching`](../../algorithms/radix-prefix-caching.md) — built on top of paged KV, shares prefixes across requests
- [`flashattention`](flashattention.md) — variable-length + paged-via-block_table API
- [`flashinfer`](flashinfer.md) — plan/run wrappers + MLA variants
- [`algorithms/chunked-prefill`](../../algorithms/chunked-prefill.md) — uses the varlen path for interleaved prefill + decode batches
