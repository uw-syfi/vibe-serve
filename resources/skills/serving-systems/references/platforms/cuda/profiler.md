...
|:--------|:-------------|:----------|
| Long gaps between kernels | Python overhead, graph breaks, launch-bound | [`algorithms/async-scheduling`](../../algorithms/async-scheduling.md), [`cuda-graph`](cuda-graph.md) |
...
4. If `cpu_launch_bound` / `python_overhead_bound` / `sync_bound` → [`algorithms/async-scheduling`](../../algorithms/async-scheduling.md), [`cuda-graph`](cuda-graph.md), [`frameworks/pytorch`](../../frameworks/pytorch.md).
5. If `input_pipeline_bound` / `memcpy_bound` → pinned memory, prefetch, overlap copies.
6. If `comm_bound` → [`algorithms/parallelism`](../../algorithms/parallelism.md), [`platforms/cuda/hardware/`](hardware.md) topology.
...
4. Recommend kernel or launch-config changes (or switch to a better attention backend — see [`attention-backend-comparison.md`](attention-backend-comparison.md)).
...
- [`tooling/serving-benchmark`](../../tooling/serving-benchmark.md) — benchmark is the *input* to the profiler; get benchmark hygiene right first
...
- [`cuda-graph`](cuda-graph.md) — common fix when nsys shows gaps between kernels
