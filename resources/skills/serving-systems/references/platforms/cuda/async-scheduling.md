...
1. **Kernel-launch-level** — addressed by CUDA graphs and fused kernels (see [`cuda-graph`](cuda-graph.md)).
...
|:-----|:--------------|:------|
| Kernel launch | per-kernel CPU launch cost (~50µs × N kernels per step) | [`cuda-graph`](cuda-graph.md) |
| Python per-op overhead | framework dispatch, .to(), .item() syncs | [`frameworks/pytorch`](../../frameworks/pytorch.md) (torch.compile) |
| Sampler sync | `.item()` per request per step | [`algorithms/batched-sampling`](../../algorithms/batched-sampling.md) |
...
- [`platforms/cuda/continuous-batching`](continuous-batching.md) — the scheduling substrate that async scheduling overlays
- [`algorithms/batched-sampling`](../../algorithms/batched-sampling.md) — removes another sync source on the same critical path
- [`cuda-graph`](cuda-graph.md) — hides kernel-launch overhead; orthogonal to async scheduling
