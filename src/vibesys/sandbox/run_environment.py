...
def _modal_runtime_notes(gpu: str, app_name: str) -> str:
    ...
    return (
        ...
        "  - `.venv` and `UV_CACHE_DIR=/workspace/.cache/uv` are ignored, "
        "persistent runtime assets: Git checkpoint restoration and editor "
        "restarts do not replace them. With unchanged dependency metadata, run "
        "Python tools through the project interpreter (for example, "
        "`.venv/bin/python -m pytest` or `.venv/bin/python -m py_compile ...`). "
        "Do not use a bare tool or `uv run --no-sync <tool>` when that tool is "
        "absent from the project: its global executable may select the wrong "
        "Python. If a test-only module such as pytest exists only in the base "
        "editor, run the base `python -m pytest` with the project's site-packages "
        "explicitly first on `PYTHONPATH`; do not ask uv to create an overlay. "
        "Synchronize once only after dependency metadata changes or a verified "
        "runtime import is missing; never delete or recreate `.venv` as ordinary "
        "diagnosis. The persistent cache makes a genuinely necessary repair "
        "incremental. Search source through `rg` or Git's file list while "
        "excluding `.venv` and `.cache`; search generated evidence only under "
        "named experiment paths, never broad roots such as `/home`, `/root`, or "
        "`/opt`.\n"
