...
The workspace contains a checkout of **vllm-omni v0.24.1** at `vllm-omni/`
(pinned via `[[workspace.sources]]`; provenance in `_vibesys_sources.json`).
Do not write the serving system from scratch: build the candidate by adapting
vllm-omni's Show-o2 / omni-modality serving path to this server contract. You
may edit, prune, or vendor parts of the checkout freely — it is ordinary
mutable workspace code.
