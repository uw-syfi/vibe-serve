...
`benchmark.py` is a thin shim over the TraceLab replay evaluator at
`_evaluator/tracelab-replay`. The evaluator downloads the pinned TraceLab
public DuckDB release, verifies its SHA256, converts it with TraceLab's CSV
exporter, and directly invokes TraceLab's Rust `session_runner`.

Run:

```bash
uv run python benchmark/benchmark.py \
  --evaluator-dir ../../evaluators/tracelab-replay \
