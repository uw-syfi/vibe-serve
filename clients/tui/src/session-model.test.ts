...
describe("event batch projection", () => {
  it("keeps the existing banner while resumed history ends in a running session", () => {
    const before = reportError(initialSessionState(), "Local input problem", {
      scope: "input",
    });
...
describe("hypothesis planning activity", () => {
  ...
  it("maps the loop's named pre, profiler, and plan phases", () => {
    expect(
      hypothesisPlanningActivity(stateFor("orchestrator", "round-3-pre")),
    ).toMatchObject({
      stage: "pre",
      roundNumber: 3,
    });
    expect(
      hypothesisPlanningActivity(stateFor("profiler", "round-3-profiler")),
    ).toMatchObject({
      stage: "profile",
      roundNumber: 3,
    });
    expect(
      hypothesisPlanningActivity(stateFor("orchestrator", "round-3-plan")),
    ).toMatchObject({
      stage: "plan",
      roundNumber: 3,
    });
  ...
  it("does not present a phase as planning after its round has a hypothesis", () => {
    ...
    expect(
      hypothesisPlanningActivity(
        stateFor("orchestrator", "round-3-plan", entries),
      ),
    ).toBeNull();
  });
  ...
  it("keeps elapsed planning time from the earliest observed planning phase", () => {
    ...
    expect(hypothesisPlanningActivity(state)?.startedAt).toBe(
  ...
  it("ignores similarly named phases outside the loop contract", () => {
    expect(
      hypothesisPlanningActivity(stateFor("profiler", "round-3-profile-retry")),
    ).toBeNull();
  });
  ...
  it("moves a selected planning row onto the hypothesis that materializes from it", () => {
    ...
    state = {
      ...
      core: {
        ...state.core,
        phases: state.core.phases.map((phase) => ({
          ...phase,
...
describe("unowned rounds", () => {
  it("opens an observed round and keeps its turns scoped to that round", () => {
    ...
    expect(
      opened && visibleConversation(opened).map((entry) => entry.content),
    ).toEqual(["kept"]);
  });
...
describe("session event model", () => {
  it("tracks concurrent agent executions independently through activity and finish events", () => {
    ...
    expect(Object.keys(state.core.activeExecutions)).toEqual([
      "impl-1",
      "review-1",
    ]);
  ...
  it("reconciles from a checkpoint without advancing the replay cursor", () => {
    ...
    expect(
      applyActiveExecutionCheckpoint(newer, [], 4).core.activeExecutions,
    ).toEqual(state.core.activeExecutions);
  });
  ...
  it("tracks and finishes chat executions before routing their transcript", () => {
    ...
    expect(
      state.core.activeExecutions["chat-execution"]?.activity.summary,
    ).toBe("Inspecting the run");
    state = applyEvent(
      state,
      executionEvent(
        2,
  ...
  it("shows structured configuration failures as terminal conversation entries", () => {
    ...
    expect(state.errorBanner?.message).toContain(
      "This run has completed 30 rounds.",
    );
    expect(state.core.transcript[0]?.content).toContain(
      "This run has completed 30 rounds.",
    );
    expect(state.core.transcript[0]?.content).toContain(
      "resume_limit_exhausted",
    );
  ...
  it("dismisses an error without changing session state or suppressing later errors", () => {
    const state = reportError(
      { ...initialSessionState(), selectedRound: 2 },
      "The request failed.",
      {
        scope: "request",
      },
    ...
    expect(
      reportError(dismissed, "A later failure.", { scope: "transport" })
        .errorBanner,
  ...
  it("routes chat agent trajectory events away from the experiment transcript", () => {
    ...
    expect(state.chatConversation.at(-1)?.content).toBe(
      "Round 2 improved throughput.",
    );
  });
  ...
  it("pairs typed tool_call and tool_result events into tool turns", () => {
    ...
    state = applyEvent(
      ...
      event(
        1,
        "tool_call",
        { kind: "tool_call", tool: "Bash", args: { command: "first" } },
        "inv-1",
      ),
    );
    state = applyEvent(
    ...
    state = applyEvent(
      ...
      event(
        3,
        "tool_call",
        { kind: "tool_call", tool: "Bash", args: { command: "second" } },
        "inv-1",
      ),
    );
    state = applyEvent(
  ...
  it("correlates parallel typed tool results by call ID", () => {
    ...
    expect(
      state.core.transcript.map((entry) => entry.toolResult?.content),
  ...
  it("retains long typed tool-call arguments for presentation", () => {
    ...
    const state = applyEvent(
      ...
      event(1, "tool_call", {
        kind: "tool_call",
        tool: "Edit",
        args: { text: longArg, count: 3 },
      }),
    );

    expect(state.core.transcript[0]?.toolArguments).toEqual({
      text: longArg,
      count: 3,
    });
  ...
  it("keeps each phase’s todo list separate so agents never clobber each other", () => {
    ...
    state = applyEvent(state, {
      ...
      data: {
        kind: "todo_update",
        todos: [{ content: "Edit files", status: "in_progress" }],
      },
    });
    state = applyEvent(state, {
      sequence: 2,
      ...
      data: {
        kind: "todo_update",
        todos: [{ content: "Check behavior", status: "pending" }],
      },
    });
    state = applyEvent(state, {
      sequence: 3,
    ...
    expect(visibleTodos(withAgent)).toEqual([
    ...
    expect(visibleTodos(withRound)).toEqual([
      { content: "Check behavior", status: "pending" },
    ]);
  });
  ...
  it("keeps todos separate for concurrent executions of the same agent role", () => {
    ...
    expect(state.core.todos.map((phase) => phase.executionId)).toEqual([
      "impl-a",
      "impl-b",
    ]);
    expect(
    ...
    ).toEqual([{ content: "Edit implementation B", status: "in_progress" }]);
  });
  ...
  it("hides todos when the active phase has not emitted any", () => {
    ...
    state = applyEvent(state, {
      ...
      data: {
        kind: "todo_update",
        todos: [{ content: "Edit files", status: "completed" }],
      },
    });
  ...
  it("preserves unknown todo statuses for the renderer to degrade", () => {
    ...
    expect(state.core.todos[0]?.items).toEqual([
      { content: "Mystery step", status: "deferred" },
    ]);
  });
  ...
  it("derives round-scoped agent flow from run and phase events", () => {
    ...
    expect(
      visiblePhases(state).map((phase) => `${phase.kind}:${phase.status}`),
    ).toEqual([
...
describe("hypothesis detail navigation", () => {
  const hypothesis = {
    ...
    identified: true,
    claim:
      "A complete causal hypothesis that must never be truncated in its detail view.",
    first_round: 1,
    last_round: 2,
    rounds: [
  ...
  it("moves from index to hypothesis to round and unwinds one level at a time", () => {
    ...
    expect(detail.hypothesisDetail).toEqual({
      entryKey: "H-01",
      selectedRound: 2,
    });
  ...
  it("closes a detail whose hypothesis disappears during refresh", () => {
    const detail = enterExperimentDrilldown(
      setExperiments(initialSessionState(), [hypothesis]),
    );
...
describe("right pane layout", () => {
  ...
  it("opens a pane pending, titled, and focused", () => {
    ...
    expect(state.layout.right).toMatchObject({
      view: "perf",
      title: "Performance",
      pending: true,
    });
  ...
  it("cycles focus left to right through the columns on screen", () => {
    ...
    expect(cyclePaneFocus(open, -1).layout.focus).toBe("left");
    expect(cyclePaneFocus(cyclePaneFocus(open, -1), -1).layout.focus).toBe(
      "chat",
    );
  ...
  it("cycles between the agent map and transcript inside a round", () => {
    const transcript = {
      ...initialSessionState(),
      experimentLog: null,
      selectedRound: 1,
    ...
    const agents = cyclePaneFocus(transcript);
    expect(agents.roundFocus).toBe("agents");
    expect(cyclePaneFocus(agents).roundFocus).toBe("transcript");
    expect(cyclePaneFocus(transcript, -1).roundFocus).toBe("agents");
  });
  ...
  it("leaves the chat transcript untouched when the pane closes", () => {
    const withChat = {
      ...
      chatOpen: true,
      chatConversation: [
        { id: "c1", kind: "user" as const, content: "why is r3 slower?" },
      ],
    };
  ...
  it("never reports the hidden agents pane as focused beside a visualization", () => {
    const agents = focusRound(
      { ...initialSessionState(), experimentLog: null },
      "agents",
    );
...
describe("theme picker", () => {
  ...
  it("clamps at both ends of the list rather than wrapping", () => {
    ...
    expect(moveThemeSelection(opened, 99).themePicker?.selected).toBe(
      "high-contrast-light",
    );
  });
  ...
  it("closes on dismissal and on applying a theme", () => {
    const opened = moveThemeSelection(
      openThemePicker(initialSessionState()),
      1,
    );
